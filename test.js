document.addEventListener("DOMContentLoaded", () => {
  ...
  questionsEl.innerHTML = questions.map((question,index) => {
    ...
    return `<article class="mcq-card" id="question-${n}" data-question-card="${n}">
      <header><span>Q${n}</span><div><small>${question.topic}</small><small>Difficulty ${question.difficulty}/5</small><small>${question.calculator}</small><b>${question.marks} mark${question.marks>1?"s":""}</b></div></header>
