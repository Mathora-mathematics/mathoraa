document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateProgress() {
    ...
    questionData.forEach((_, index) => {
      ...
      const done = Boolean(item.answer?.trim() || workingUploads[number] || item.whiteboard);
  ...
  async function uploadWorkingFiles(attemptId) {
    ...
    for (let number = 1; number <= 12; number += 1) {
      if (workingUploads[number]) {
        const file = workingUploads[number];
